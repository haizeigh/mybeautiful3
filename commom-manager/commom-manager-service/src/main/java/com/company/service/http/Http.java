...
public class Http {
    ...
    /**
     * 发起请求，如果请求超时或者失败，会立刻循环重试，写死了循环3次，
     * */
    public String requestWithRetry(){
        LOGGER.info("开始请求,url:" + this.url);
        ...
        for (int i = 0;i < 3 ;i++){
            try{
                ...
                if (i < 2){
                    LOGGER.info("网络异常,即将重试,请求url:" + this.url);
                }else {
                    LOGGER.error("网络异常，请求url:" + this.url,e);
                    throw e;
                }
            }
        }
        long endTime = System.currentTimeMillis();
        LOGGER.info("请求结束,url:" + this.url + "耗时" + (endTime-startTime) + "ms");
        return result;
    }

    public <T> T request(Type type){
        Gson gson = new Gson();
