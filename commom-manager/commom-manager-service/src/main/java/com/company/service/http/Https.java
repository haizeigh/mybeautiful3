...
public class Https {
    ...
    private String doRequest() {
        ...
        try {
            InputStream in = respCode == HttpURLConnection.HTTP_OK ?
     ...
     * @return
     */
    public String requesttCharset(String charsetName) {
        prepareRequest();
        return doRequestCharset(charsetName);
    }
    private String doRequestCharset(String charsetName) {
        if (!Strings.isNullOrEmpty(body)) {
            try {
                OutputStream out = connection.getOutputStream();
                byte[] bodyBytes = body.getBytes();
